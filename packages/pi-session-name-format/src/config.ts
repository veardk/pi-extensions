...
import { DEFAULT_CONFIG, DEFAULT_TEMPLATES } from "./types.ts";
...
function parseTemplates(raw: unknown): FormatTemplate[] | undefined {
  if (!Array.isArray(raw)) return undefined;
  const parsed: FormatTemplate[] = [];
  for (const item of raw) {
    if (
      item &&
      typeof item === "object" &&
      typeof (item as any).id === "string" &&
      typeof (item as any).template === "string"
    ) {
      parsed.push({
        id: (item as any).id,
        label:
          typeof (item as any).label === "string" ? (item as any).label : (item as any).template,
        template: (item as any).template,
      });
    }
  }
...
export function parseConfig(raw: Record<string, unknown>): NameFormatConfig {
  const ref = parseModelRef(raw.model);
  const formatMode =
    raw.formatMode === "template" || raw.formatMode === "custom" ? raw.formatMode : "custom";
  const templates = parseTemplates(raw.templates) ?? DEFAULT_TEMPLATES;
  const selectedTemplate =
    typeof raw.selectedTemplate === "string" && raw.selectedTemplate
      ? raw.selectedTemplate
      : (templates[0]?.id ?? "type-tag");

  return {
    enabled: typeof raw.enabled === "boolean" ? raw.enabled : DEFAULT_CONFIG.enabled,
    provider: typeof raw.provider === "string" && raw.provider ? raw.provider : ref.provider,
    model: typeof raw.modelId === "string" && raw.modelId ? raw.modelId : ref.model,
    maxLength:
    ...
    templates,
    formatPrompt:
      typeof raw.formatPrompt === "string" ? raw.formatPrompt : DEFAULT_CONFIG.formatPrompt,
  };
}
...
export function saveConfig(config: NameFormatConfig, agentDir: string = getAgentDir()): void {
  const configPath = path.join(agentDir, "config", CONFIG_FILENAME);
  fs.mkdirSync(path.dirname(configPath), { recursive: true });
  const out: Record<string, unknown> = {
    enabled: config.enabled,
    model: config.provider && config.model ? `${config.provider}/${config.model}` : "",
    language: config.language,
    formatMode: config.formatMode ?? "custom",
...
export function loadConfig(cwd?: string): NameFormatConfig {
  if (cwd) {
    const projectRaw = readJson(path.join(cwd, ".pi", "config", CONFIG_FILENAME));
    if (isNonEmpty(projectRaw)) return parseConfig(projectRaw);
  }
  const globalRaw = readJson(path.join(getAgentDir(), "config", CONFIG_FILENAME));
  if (isNonEmpty(globalRaw)) return parseConfig(globalRaw);

  const agentSettings = readJson(path.join(getAgentDir(), "settings.json"));
  const projectSettings = cwd ? readJson(path.join(cwd, ".pi", "settings.json")) : {};
  const block =
    (projectSettings?.piSessionNameFormat as Record<string, unknown> | undefined) ??
    (projectSettings?.nameFormat as Record<string, unknown> | undefined) ??
    (agentSettings.piSessionNameFormat as Record<string, unknown> | undefined) ??
